...
export const PollutersListVisual = ({data}) => {
    ...
    const displayName = (el) => {
        return <Typography variant="h6" component="h2" className={classes.block}>{el.Rank} <strong>{el.Name}</strong></Typography>;
    }

    const displayLocationInfos = (el) => {
        return (
            <>
                {get(el, 'HQ') ? <LabelElement><StyledLabel>HQ Location:</StyledLabel> {el.HQ}</LabelElement> : ""}
                {get(el, 'Ownership') ? <LabelElement><StyledLabel>Ownership:</StyledLabel> {el.Ownership}</LabelElement> : ""}
            </>
        )
    }
    ...
    const displayCEOInfos = (el) => {
        return (
            <>
                <LabelElement>{get(el, 'CEO pay') ? <><StyledLabel>CEO</StyledLabel>: {el.CEO} - pay: {el["CEO pay"]}</> : <><StyledLabel>CEO</StyledLabel>: {el.CEO}</>}</LabelElement>
            </>
        )
    }
    ...
    const displayInfos = (el, key) => {
        return (
            <>
                {get(el, key) ? <LabelElement><StyledLabel>{key}</StyledLabel>: {el[key]}</LabelElement> : ''}
            </>
        )
    }
                    ...
                    return (
                        <>
                            <ListItem alignItems="flex-start" divider key={el}>
                                <ListItemText primary={displayName(el)}
                                              secondary={
                                                  <>
                                                      <StyledArea>
