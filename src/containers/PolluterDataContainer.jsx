...
import {
  split, last, trim, get, words, head,
} from 'lodash';
...
const reComposeString = (string) => {
  const stringToArray = words(string);
  return `${stringToArray[0]}.${stringToArray[1]}`;
};

const parseData = (data) => data.map((element) => {
  const newData = newDataValues;
  if (get(element, 'Name')) {
    const nation = split(element.Name, ',');
  ...
  if (get(element, dataKeys.GLOBAL_EMISSION_PAST)) {
    if (element[dataKeys.GLOBAL_EMISSION_PAST].includes('tonnes of CO2')) {
      const geData = split(element[dataKeys.GLOBAL_EMISSION_PAST], 'CO2');
